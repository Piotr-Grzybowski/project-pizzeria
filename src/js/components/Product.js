    ...
    for (let param in thisProduct.data.params) {
      for (let option in thisProduct.data.params[param].options) {
        const isSelected =
          ...
          formData[param].indexOf(option) > -1; // if option selected
        const isDefault =
          thisProduct.data.params[param].options[option].default == true; // if option default
        const optionPrice =
          thisProduct.data.params[param].options[option].price; // price of option
